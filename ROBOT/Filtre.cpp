...
void Filtre::in(float newRaw,float dt)
{
    ...
    if (dt>0.5 or abs(newRaw-lastRaw)>1.5)
    {
        lastValue=newRaw;    //Le filtre n'a pas ete appelle pendant trop de temps ou il y a eu une discontinuite (modulo PI) il n'a plus de sens donc on le reset
        lastLastValue=newRaw;
        lastRaw=newRaw;
    }
    else
    {
        ...
        case 0:
        {
            lastValue=newRaw;    //A l'ordre 0, on ne fait rien
            break;
        }
        case 1:
        {
