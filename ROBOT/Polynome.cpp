...
{
    if (t<t1)
        return uIni+duIni*t+(duMax-duIni)*t*t/(2*t1);
    if (t<t2)
        return uIni+t1*(duIni+duMax)/2+duMax*(t-t1);
    if (t<t3)
        return uIni+t1*(duIni+duMax)/2+duMax*(t2-t1)+duMax*(t-t2)+(duFin-duMax)/(t3-t2)*( t*t/2-t2*t+t2*t2/2);
    return uFin;
}
...
{
    if (t<t1)
        return duIni+(duMax-duIni)*t/t1;
    if (t<t2)
        return duMax;
    if (t<t3)
        return duMax+(duFin-duMax)/(t3-t2)*(t-t2);
    return duFin;
}
...
{
    if (t<t1)
        return (duMax-duIni)/t1;
    if (t<t2)
        return 0.0;
    if (t<t3)
        return (duFin-duMax)/(t3-t2);
    return 0.0;
}
